<template>
  <view class="container" v-if="!isRelease">
    <camera devicePosition="back" flash="off" v-if="show"></camera>
    ...
    <cover-view class="showWrap co">
      <button @tap="showFun" class="ph" v-if="!show && !isTake">
        开摄像头
      </button>
    ...
    <cover-view class="takeWrap co">
      <button @tap="takePhoto" class="take" v-if="!isTake && show">
        拍 照
      </button>
      <button @tap="takePhoto" class="take" v-if="isTake && !show">
        重 拍
...
export default {
  ...
  onLoad: function () {
    ...
    t.windowHeight > 1.6 * t.windowWidth
      ? (this.w = 0.8 * t.windowWidth)
      : (this.w = t.windowHeight / 2);
    ...
    this.bgcanvas.drawImage(
      "http://xue.ccy1994.top/xiaochengxu/mini-tools/static/pages/my_module/protractor/BackGround.jpg",
      0,
      0,
      400,
      1000,
  ...
  methods: {
    ...
    drawPointer: function () {
      ...
      t.moveTo(0, 0);
      t.lineTo(
        0.84 * a * Math.cos((i / 180) * Math.PI),
        0.84 * a * Math.sin((i / 180) * Math.PI),
      );
      t.stroke();
      t.beginPath();
      t.arc(
        0.87 * a * Math.cos((i / 180) * Math.PI),
        0.87 * a * Math.sin((i / 180) * Math.PI),
        0.03 * a,
        0,
        2 * Math.PI,
      );
      ...
      t.fill();
      t.moveTo(
        0.9 * a * Math.cos((i / 180) * Math.PI),
        0.9 * a * Math.sin((i / 180) * Math.PI),
      );
      t.lineTo(
        0.95 * a * Math.cos((i / 180) * Math.PI),
        0.95 * a * Math.sin((i / 180) * Math.PI),
      );
      t.stroke();
    ...
    showFun: function () {
      ...
      false !== uni.getStorageSync("isAuth")
        ? (true === uni.getStorageSync("isAuth") &&
            (this.setData({
              show: true,
            }),
            this.bgcanvas.clearRect(0, 0, this.width, this.h),
            this.drawbg()),
