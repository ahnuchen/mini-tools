  ...
  for (let j = 0; j < months; j++) {
    ...
    if (loadDatas.length === 1) {
      ...
      for (let k = 0; k < loadDatas.length; k++) {
        ...
        let interestItem = monthdetailFunc(
          surplusArr[k],
          monthMoney,
          monthRate,
        );
...
const calculatePaymentDetail = (mortgage) => {
  ...
  let monthSYRate = mortgage.businessLoanRate / 12;
  let monthGJJRate = mortgage.gjjLoanRate / 12;
  let loadDatas = [];
  switch (mortgage.loanType) {
    case "1":
      ...
      if (mortgage.paymentMethod === 1) {
        monthlyPayment = equalPrincipalAndInterest(
          mortgage.businessTotalLoan,
          paymentMonth,
          monthSYRate,
        );
        loadDatas.push([
          getInterestMonthDetail,
          mortgage.businessTotalLoan,
          monthlyPayment,
          monthSYRate,
        ]);
        payDetails = getMonthDetails(
          paymentMonth,
          mortgage.startDate,
          loadDatas,
        );
        ...
        //等额本金
        let principal = (mortgage.businessTotalLoan / paymentMonth) * 1;
        monthlyPayment = equalPrincipal(
          mortgage.businessTotalLoan,
          paymentMonth,
          monthSYRate,
        );
        loadDatas.push([
          getPrincipalMonthDetail,
          mortgage.businessTotalLoan,
          principal,
          monthSYRate,
        ]);
    ...
    case "2":
      ...
      if (mortgage.paymentMethod === 1) {
        ...
        //等额本金
        let principal = (mortgage.gjjTotalLoan / paymentMonth) * 1;
        monthlyPayment = equalPrincipal(
          mortgage.gjjTotalLoan,
          paymentMonth,
          monthGJJRate,
    ...
    case "3":
      ...
      if (mortgage.paymentMethod === 1) {
        //商业
        let syMonthlyPayment = equalPrincipalAndInterest(
          mortgage.businessTotalLoan,
          paymentMonth,
          monthSYRate,
        );
        //公积金
        let gjjMonthlyPayment = equalPrincipalAndInterest(
          mortgage.gjjTotalLoan,
          paymentMonth,
          monthGJJRate,
        );
        ...
        //等额本金
        //商业
        let syPrincipal = (mortgage.gjjTotalLoan / paymentMonth) * 1;
        let syMonthlyPayment = equalPrincipal(
          mortgage.businessTotalLoan,
          paymentMonth,
          monthSYRate,
        );
        //公积金
        let gjjPrincipal = (mortgage.gjjTotalLoan / paymentMonth) * 1;
        let gjjMonthlyPayment = equalPrincipal(
          mortgage.gjjTotalLoan,
          paymentMonth,
          monthGJJRate,
        );
