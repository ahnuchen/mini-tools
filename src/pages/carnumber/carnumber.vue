<template>
  <view class="carnumber-wrapper">
    <wd-sidebar :model-value="active" custom-class="sidebar" @change="handleChange">
      <wd-sidebar-item custom-class="side-item" v-for="(item, index) in areas" :key="item.key" :value="index"
                       :label="item.title"/>
    </wd-sidebar>
    <scroll-view class="scroll-content" scroll-y scroll-with-animation :scroll-top="scrollTop" :throttle="false"
                 @scroll="onScroll" enable-back-to-top>
      <view v-for="(item, index) in areas" :key="index" class="category">
        <wd-cell-group :title="item.title" border>
          <wd-cell v-for="(cell, index) in item.items" :key="index" :value="cell.title" :title="cell.label">
          </wd-cell>
        </wd-cell-group>
      </view>
    </scroll-view>
